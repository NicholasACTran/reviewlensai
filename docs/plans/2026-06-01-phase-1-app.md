...
These hand-rolled types are the FE's **view** of the contract (spec §3) — deliberately a subset: the Amplify-managed `createdAt`/`updatedAt`/`expiresAt` exist on the row but the FE doesn't render them, so they're omitted here. The Amplify-generated `Schema` type must structurally cover this subset (asserted in Task 3).
...
```
> The throw is fine because `getJobClient()` is now called lazily (inside `App`'s render, only on the non-fake path). Task 7 route tests import the routes directly and never trigger it.
...
`app/src/main.tsx` — configure Amplify only when not in fake mode, guarding the generated file (gitignored, absent in local dev). Add before render:
```tsx
import { Amplify } from "aws-amplify";
if (import.meta.env.VITE_USE_FAKE !== "true") {
  // amplify_outputs.json is produced by the Amplify pipeline; absent locally. Dynamic, non-static
  // specifier so the bundler doesn't fail import-analysis when the file is missing.
  const outputs = (await import(/* @vite-ignore */ "../amplify_outputs.json")).default;
  Amplify.configure(outputs as Record<string, unknown>);
}
```
> `module: "ESNext"` (Task 1 tsconfig) permits top-level `await`, so `tsc -b` is clean.

`app/src/api/jobClient.select.ts` (replace body — static ESM import, no `require`):
```ts
...
export function getJobClient(): JobClient {
  if (config.useFake) return new FakeJobClient();
  return new AmplifyJobClient(generateClient().models); // called lazily by App (non-fake path only)
}
```
...
jobs:
  deploy:
    ...
    steps:
      ...
      - name: Publish Amplify hosting URL to SSM (read from the API, never string-templated)
        run: |
          DOMAIN=$(aws amplify get-app --app-id ${{ secrets.AMPLIFY_APP_ID }} --query 'app.defaultDomain' --output text)
          URL="https://main.${DOMAIN}"
          # Fail loudly if the URL is malformed — it becomes the Validator CORS origin (spec §6).
          case "$URL" in https://main.*.amplifyapp.com) ;; *) echo "Bad Amplify URL: $URL" && exit 1 ;; esac
          aws ssm put-parameter --name /reviewlensai/amplify/url --type String --overwrite --value "$URL"
```
> CI gate is the same `lint && typecheck && test` you run locally (spec §9). The hosting URL is read from `amplify get-app .app.defaultDomain` (the real branch host = `main.<defaultDomain>`) rather than string-templating `<appId>`, because this exact value is baked into the scraper's Function URL CORS allow-list at synth — a wrong value silently CORS-blocks every browser POST and needs a full scraper redeploy to fix. A custom domain would be read the same way.
...
- **Blocker — `require()` under ESM:** replaced with a static `import { generateClient }`; client instantiation made **lazy** in `App` so importing it in tests/build never throws (Task 7/8).
- **Blocker — hardcoded Amplify hosting URL:** now read from `amplify get-app .app.defaultDomain` with a format assertion, because it's the scraper's CORS origin (Task 9).
- **Concerns:** removed the discarded `Amplify.configure` snippet (guarded dynamic import shown directly); stray `why` token deleted from `UrlBoxPage`; `Job` type reframed as the "FE view subset," not "source of truth."
- **Naming:** the class is `FakeJobClient`; prose/spec also call the concept "FakeAmplifyClient" — same thing, `FakeJobClient` is the implementation name.
